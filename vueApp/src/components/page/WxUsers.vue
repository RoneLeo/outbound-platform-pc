    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><i class="el-icon-lx-cascades"></i> 微信用户信息</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
        ...
        <div class="container">
            <el-table :data="datavalues" :border="true"
                ...
                <el-table-column prop="sqm" label="授权码" :show-overflow-tooltip="false" width="80"
                                 :sortable="false" align="left"></el-table-column>
                <el-table-column prop="sqm" label="是否授权" :show-overflow-tooltip="false" width="100" :formatter="SfsqFormatter"
...
<script>
    export default {
        ...
        methods: {
            ...
            SfsqFormatter(row) {
                return row.bdsj && row.bdsj.length  ? '是' : '否';
            },
            JsFormatter(row) {
                return this.$common.dictParse(row.js, this.roles);
            },
            getData() {
                this.$axios.post('/user/findAll', {lx:1, zt: this.dictType, page: this.currentPage, pagesize: this.pageSize}).then((res) => {
                    if(res.resCode == 200){
